...
async function createSession(page, name = "Alice") {
  ...
  await expect(
    page.getByRole("button", { name: "Claim keyboard" }),
  ).toBeVisible();
...
async function sessionPost(page, body) {
  const payload = { ...body };
  if (payload.clientId && !payload.clientKey)
    payload.clientKey = clientKeyFor(payload.clientId);
...
async function sessionGet(page, params) {
  ...
  if (query.clientId && !query.clientKey) {
    try {
      ...
      if (client.clientId === query.clientId && client.clientKey)
        query.clientKey = client.clientKey;
    } catch {}
  }
  if (query.clientId && !query.clientKey)
    query.clientKey = clientKeyFor(query.clientId);
  const res = await page.request.get(
    `/api/session?${new URLSearchParams(query)}`,
  );
  expect(res.ok()).toBeTruthy();
  return res.json();
}
...
function callSessionHandler(
  ...
  return new Promise((resolve) => {
    ...
    const nextBody = { ...body };
    if (nextQuery.clientId && !nextQuery.clientKey)
      nextQuery.clientKey = clientKeyFor(nextQuery.clientId);
    if (nextBody.clientId && !nextBody.clientKey)
      nextBody.clientKey = clientKeyFor(nextBody.clientId);
...
test("creating a session stays on the session screen after polling", async ({
  page,
}) => {
...
test("polling does not replace the header while the timer is ticking", async ({
  page,
}) => {
  await createSession(page);
...
test("contest timer changes on second boundaries and freezes while paused", async ({
  page,
}) => {
  await createSession(page);
...
test("contest timer ignores a client wall-clock adjustment", async ({
  page,
}) => {
  await createSession(page);
  const initial = parseClock(await page.locator("[data-timer]").innerText());
  await page.getByRole("button", { name: "Start timer" }).click();

  await page.evaluate(() => {
    const nativeDateNow = Date.now;
    Date.now = () => nativeDateNow() + 60 * 60 * 1000;
  });
  await page.waitForTimeout(1_200);

  const displayed = parseClock(await page.locator("[data-timer]").innerText());
  expect(displayed).toBeLessThan(initial);
  expect(displayed).toBeGreaterThanOrEqual(initial - 2);
});

test("contest timer immediately catches up when a hidden tab becomes visible", async ({
  page,
}) => {
  await createSession(page);
  await page.getByRole("button", { name: "Start timer" }).click();
  const beforeHidden = parseClock(
    await page.locator("[data-timer]").innerText(),
  );
  ...
  const hiddenText = await page.locator("[data-timer]").innerText();
  await page.waitForTimeout(1_100);
  await expect(page.locator("[data-timer]")).toHaveText(hiddenText);
  ...
  await expect
    .poll(async () =>
      parseClock(await page.locator("[data-timer]").innerText()),
    )
    .toBeLessThan(beforeHidden);
});

test("expanded last action stays open across polling and countdown updates", async ({
  page,
}) => {
  await createSession(page);
  await page.getByRole("button", { name: "Start timer" }).click();
  ...
  await expect(page.locator(".scroll-log")).toContainText(
...
test("claim and release update the status and audit log", async ({ page }) => {
  ...
  await page
    .getByLabel("Optional claim note")
    .fill("Taking over input parsing");
  await page.getByRole("button", { name: "Claim keyboard" }).click();
  await expect(page.locator(".holder")).toHaveText("Alice");
  await expect(
    page.getByRole("button", { name: "Release keyboard" }),
  ).toBeVisible();
  await expect(page.locator("[data-audit-log] summary")).toContainText(
    'Alice claimed the keyboard: "Taking over input parsing"',
  );
  await page.getByRole("button", { name: "Add note…" }).click();
  await page
    .getByLabel("Optional release note")
    .fill("Handing off after fixing the parser");
  await page.getByRole("button", { name: "Release keyboard" }).click();
  await expect(page.locator(".holder")).toHaveText("Unused");
  await page.locator("[data-audit-log] summary").click();
  await expect(page.locator(".log")).toContainText(
    'released the keyboard: "Handing off after fixing the parser"',
  );
});
...
test("leaving a session returns the tab to the home page", async ({ page }) => {
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole("dialog", { name: "Leave session" }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "Cancel" })).toHaveCount(0);
  await page
    .getByRole("dialog", { name: "Leave session" })
...
test("two teammates see claims and requests through polling", async ({
  browser,
}) => {
  ...
  await expect(
    bob.getByRole("button", { name: "Claim keyboard" }),
  ).toBeVisible();
  ...
  await expect(alice.locator("[data-audit-log] summary")).toContainText(
    "Bob requested the keyboard",
    { timeout: 5000 },
  );
...
test("join rejects a duplicate teammate name", async ({ browser, page }) => {
  ...
  await expect(bob.locator(".toast")).toContainText(
    'Error: A teammate named "Alice" is already in this session',
  );
...
test("client-side settings tampering cannot rename the current member", async ({
  page,
}) => {
  const code = await createSession(page, "Alice");
  const client = await page.evaluate(() => ({
  ...
  expect(tampered.session.events.at(-1).message).toContain(
    "Alice updated settings",
  );
  await expect(page.locator(".topbar")).toContainText("Alice");
});

test("server rejects spoofing another member with a mismatched client key", async ({
  page,
}) => {
...
test("requesting from a holder notifies the holder and does not steal the keyboard", async ({
  browser,
}) => {
  const aliceContext = await browser.newContext({
    permissions: ["notifications"],
  ...
  await bob.getByRole("button", { name: "Join Session" }).click();
  await expect(
    bob.getByRole("button", { name: "Request keyboard" }),
  ).toBeVisible();
  ...
  await bob
    .getByLabel("Optional request note")
    .fill("Need to test the fix on problem C");
  await bob.getByRole("button", { name: "Request keyboard" }).click();
  await expect(alice.locator("[data-audit-log] summary")).toContainText(
    'Bob requested the keyboard: "Need to test the fix on problem C"',
    { timeout: 5000 },
  );
  await expect(alice.locator(".holder")).toHaveText("Alice");
  await expect(bob.locator(".holder")).toHaveText("Alice");
  await expect(alice.locator(".request-note")).toContainText(
    "Need to test the fix on problem C",
  );
  await expect
    .poll(() =>
      alice.evaluate(() => window.__notifications.map((item) => item.body)),
    )
    .toContain(
      'Bob requested the keyboard: "Need to test the fix on problem C"',
    );
...
test("holder can release keyboard to requester from popup", async ({
  browser,
}) => {
  const aliceContext = await browser.newContext();
  const bobContext = await browser.newContext();
  ...
  await bob
    .getByLabel("Optional request note")
    .fill("Ready to drive the next submission");
  await bob.getByRole("button", { name: "Request keyboard" }).click();

  await expect(alice.locator(".request-note")).toContainText(
    "Ready to drive the next submission",
  );
...
test("holder can reject requester popup and timeout auto-rejects", async ({
  ...
  await bob.getByRole("button", { name: "Request keyboard" }).click();
  await expect(
    alice.getByRole("dialog", { name: "Keyboard request" }),
  ).toBeVisible({ timeout: 5000 });
...
test("request countdown stays stable and refreshes once when it reaches zero", async ({
  browser,
}) => {
  const aliceContext = await browser.newContext();
  const bobContext = await browser.newContext();
  ...
  const aliceClientId = await alice.evaluate(() =>
    localStorage.getItem("clientId"),
  );
  await expect
    .poll(async () => {
      const session = await sessionGet(alice, {
        code,
  ...
  const sameDialog = await alice.evaluate(
    () => window.__requestDialog === document.querySelector(".request-pop"),
  );
  ...
  const stillRemoved = await alice.evaluate(
    () => window.__requestDialog === document.querySelector(".request-pop"),
  );
...
test("three users cannot concurrently claim or release another holder", async ({
  browser,
}) => {
  const aliceContext = await browser.newContext();
  const bobContext = await browser.newContext();
  ...
  for (const [page, name] of [
    ...
    await expect(
      page.getByRole("button", { name: "Request keyboard" }),
    ).toBeVisible();
    await expect(page.locator(".holder")).toHaveText("Alice");
  }
  ...
  await expectBadPost(
    bob,
    { action: "claim", code, clientId: "bob-api", name: "Bob" },
    "Session access denied",
  );
  await expectBadPost(
    charlie,
    { action: "release", code, clientId: "charlie-api", name: "Charlie" },
    "Session access denied",
  );
  await expect(alice.locator(".holder")).toHaveText("Alice");
...
test("leave action removes the teammate and force releases the keyboard", async ({
  page,
}) => {
  const created = await sessionPost(page, {
    action: "create",
  ...
  const left = await sessionPost(page, {
    action: "leave",
    code,
    clientId: "owner",
    name: "Owner",
  ...
  expect(left.session.events.at(-1).message).toContain(
    "left and released the keyboard",
  );
});

test("any teammate can kick a stuck holder through the existing leave cleanup", async ({
  browser,
}) => {
  ...
  const bobClientId = await bob.evaluate(() =>
    localStorage.getItem("clientId"),
  );
  await owner
    .locator(`[data-client-id="${bobClientId}"]`)
    .click({ force: true });
  await expect(
    owner.getByRole("dialog", { name: "Kick teammate" }),
  ).toBeVisible();
  await owner.getByRole("button", { name: "Kick teammate" }).click();

  await expect(owner.locator(".holder")).toHaveText("Unused");
  await expect(owner.locator(".members")).not.toContainText("Bob", {
    timeout: 5000,
  });
  await expect(owner.locator("[data-audit-log] summary")).toContainText(
    "Bob was kicked by Owner and released the keyboard",
  );
...
test("kicking a pending requester clears the pending request", async ({
  page,
}) => {
  const created = await sessionPost(page, {
    action: "create",
  ...
  const kicked = await sessionPost(page, {
    action: "kick",
    code,
    clientId: "owner",
    name: "Owner",
  ...
  expect(kicked.session.events.at(-1).message).toContain(
    "Guest was kicked by Owner",
  );

  await expectBadPost(
    page,
    { action: "request", code, clientId: "guest", name: "Guest" },
    "You were removed from this session",
  );
  const rejoined = await sessionPost(page, {
    action: "join",
    code,
    clientId: "guest",
    name: "Guest",
...
test("session storage expires sessions using the configured TTL", async () => {
  ...
  try {
    ...
    if (previousTtl === undefined)
      delete process.env.ONE_KEYBOARD_SESSION_TTL_SECONDS;
...
test("leaving from teammates removes the member for everyone else", async ({
  browser,
}) => {
  const ownerContext = await browser.newContext();
  const bobContext = await browser.newContext();
  ...
  await bob
    ...
    .click();
  await expect(
    bob.getByRole("button", { name: "Create Session" }),
  ).toBeVisible();
  await expect(owner.locator(".members")).not.toContainText("Bob", {
    timeout: 5000,
  });
...
test("closing while only the countdown is running still asks for confirmation", async ({
  browser,
}) => {
...
test("closing an active page asks for confirmation without removing the holder from the session", async ({
  browser,
}) => {
  const aliceContext = await browser.newContext();
  const bobContext = await browser.newContext();
  ...
  await expect(bob.locator(".holder")).toHaveText("Alice", { timeout: 5000 });
  await expect(bob.locator(".members")).toContainText("Alice", {
    timeout: 5000,
  });
  ...
  expect(
    Object.values(session.session.members).map((member) => member.name),
  ).toContain("Alice");

  await bobContext.close();
});

test("timer settings do not steal focus while countdown updates", async ({
  page,
}) => {
  await createSession(page);
  ...
  await page.evaluate(() => {
    ...
    window.__settingsHours = document.querySelector(
      '.settings input[name="hours"]',
    );
  });
  ...
  const stable = await page.evaluate(() => ({
    ...
    sameHours:
      window.__settingsHours ===
      document.querySelector('.settings input[name="hours"]'),
  }));
  expect(stable.sameModal).toBe(true);
  expect(stable.sameHours).toBe(true);
});

test("poll failure does not send the user back to the homepage", async ({
  page,
}) => {
  await createSession(page);
...
test("claim still works after a transient poll failure", async ({ page }) => {
  ...
  await page.route("**/api/session?code=*", (route) => {
    if (failPolls && route.request().method() === "GET") {
      route.fulfill({
        status: 503,
        contentType: "application/json",
        body: '{"error":"Temporary sync failure"}',
      });
    } else {
      route.continue();
    }
  ...
  await expect(page.locator("[data-audit-log] summary")).toContainText(
    "Alice claimed the keyboard",
    { timeout: 5000 },
  );
});

test("direct join link pre-fills the code and lets a teammate join", async ({
  page,
  browser,
}) => {
...
test("settings save updates name and timer without stale sync warnings", async ({
  page,
}) => {
  await createSession(page);
  await page.getByRole("button", { name: "Settings" }).click();
...
test("settings copy icon writes the join link to clipboard", async ({
  page,
}) => {
...
test("settings reports a clipboard write failure instead of a false success", async ({
  page,
}) => {
  await createSession(page);
  await page.evaluate(() => {
  ...
  await page.getByRole("button", { name: "Settings" }).click();
  await page.getByRole("button", { name: "Copy join link" }).click();
  await expect(page.locator(".toast")).toContainText("Error: Could not copy.");
});

test("settings cog opens and close button closes the modal", async ({
  page,
}) => {
  await createSession(page);
  await page.getByRole("button", { name: "Settings" }).click();
...
test("escape closes settings and a non-holder sees accurate leave guidance", async ({
  browser,
}) => {
  const ownerContext = await browser.newContext();
  ...
  await expect(
    guest.getByRole("dialog", { name: "Leave session" }),
  ).toContainText("Your presence will be removed from the session.");

  await ownerContext.close();
  await guestContext.close();
});

test("teammate list stays compact and shows active members", async ({
  browser,
}) => {
  const owner = await browser.newPage();
  const bob = await browser.newPage();
...
test("teammate list is sorted alphabetically", async ({ browser }) => {
  ...
  await expect(owner.locator(".members .member")).toHaveCount(4, {
    timeout: 5000,
  });
  const names = await owner
    .locator(".members .member > span:first-child")
    .evaluateAll((nodes) => nodes.map((node) => node.textContent.trim()));
  expect(names).toEqual(["Amy", "Mike", "Owner", "Zoe"]);
});

test("member presence badges move from online to idle without replacing the badge", async ({
  browser,
}) => {
  const ownerContext = await browser.newContext();
  const bobContext = await browser.newContext();
  ...
  await owner.evaluate((clientId) => {
    window.__bobBadge = document.querySelector(
      `[data-member-status="${clientId}"]`,
    );
  }, bobClientId);

  await owner.waitForTimeout(12500);
  await expect(bobBadge).toHaveText("idle", { timeout: 3000 });
  const stable = await owner.evaluate(
    (clientId) =>
      window.__bobBadge ===
      document.querySelector(`[data-member-status="${clientId}"]`),
    bobClientId,
  );
...
test("pause freezes contest elapsed time and resume restarts from the same audit timestamp", async ({
  page,
}) => {
  const created = await sessionPost(page, {
    action: "create",
  ...
  await sleep(1200);
  const stopped = await sessionPost(page, {
    action: "timer",
    command: "stop",
    code,
    clientId: "owner",
  ...
  await sleep(1200);
  const resumed = await sessionPost(page, {
    action: "timer",
    command: "start",
    code,
    clientId: "owner",
  ...
  await sleep(1100);
  const joined = await sessionPost(page, {
    action: "join",
    code,
    clientId: "bob-resume",
    name: "Bob Resume",
...
test("audit rows keep their original local or relative labels across start reset and restart", async ({
  page,
}) => {
  const code = await createSession(page);
  ...
  const claimed = await sessionPost(page, {
    action: "claim",
    code,
    ...owner,
    name: "Alice",
  ...
  expect(
    eventByMessage(claimed.session, "claimed the keyboard").timestampMode,
  ).toBe("local");
  ...
  await sleep(1400);
  const firstRelative = await sessionPost(page, {
    action: "join",
    code,
    clientId: "bob-cycle",
    name: "Bob Cycle",
  ...
  const localAfterReset = await sessionPost(page, {
    action: "join",
    code,
    clientId: "charlie-cycle",
    name: "Charlie Cycle",
  });
  const charlieEvent = eventByMessage(
    localAfterReset.session,
    "Charlie Cycle joined",
  );
  ...
  await sleep(1400);
  const secondRelative = await sessionPost(page, {
    action: "join",
    code,
    clientId: "dana-cycle",
    name: "Dana Cycle",
  ...
  const bobRow = page
    .locator(".log-row")
    .filter({ hasText: "Bob Cycle joined" })
    .first();
  const charlieRow = page
    .locator(".log-row")
    .filter({ hasText: "Charlie Cycle joined" })
    .first();
  const danaRow = page
    .locator(".log-row")
    .filter({ hasText: "Dana Cycle joined" })
    .first();
...
test("api stores optional notes on claim request and release events", async ({
  page,
}) => {
  const created = await sessionPost(page, {
    action: "create",
  ...
  const claimed = await sessionPost(page, {
    ...
    clientId: "owner",
    name: "Alice",
    note: "Taking over editor navigation",
  });
  expect(
    eventByMessage(claimed.session, "claimed the keyboard").message,
  ).toContain('Alice claimed the keyboard: "Taking over editor navigation"');
  ...
  const requested = await sessionPost(page, {
    action: "request",
    code,
    clientId: "bob",
    name: "Bob",
    note: "Need the keyboard for the final submit",
  });
  expect(requested.session.pendingRequest.note).toBe(
    "Need the keyboard for the final submit",
  );
  expect(
    eventByMessage(requested.session, "requested the keyboard").message,
  ).toContain(
    'Bob requested the keyboard: "Need the keyboard for the final submit"',
  );

  const released = await sessionPost(page, {
    action: "release",
    code,
    clientId: "owner",
    name: "Alice",
...
test("invalid join code reports an error and keeps form usable", async ({
  page,
}) => {
  await page.goto("/");
...
test("api validation covers invalid codes actions users and duration clamping", async ({
  page,
}) => {
  await expectBadPost(
    page,
    { action: "join", code: "BAD", clientId: "x", name: "X" },
    "Invalid session code",
  );
  await expectBadPost(
    page,
    { action: "create", clientId: "", name: "X" },
    "Missing client id",
  );
  await expectBadPost(
    page,
    { action: "create", clientId: "x", name: "" },
    "Missing name",
  );
  ...
  await expectBadPost(
    page,
    { action: "nope", code, clientId: "owner", name: "Owner" },
    "Unknown action",
  );
  await expectBadPost(
    page,
...
test("API canonicalizes authenticated client ids before mutating holder state", async ({
  page,
}) => {
  const created = await sessionPost(page, {
    action: "create",
  ...
  const claimed = await sessionPost(page, {
    action: "claim",
    code,
    // Authorization intentionally accepts surrounding whitespace; state must
    // still use the canonical id so this holder remains releasable.
    clientId: " owner ",
    clientKey: clientKeyFor("owner"),
    name: "Not the owner",
  });
  expect(claimed.session.holder.clientId).toBe("owner");
  expect(claimed.session.holder.name).toBe("Owner");
  expect(claimed.session.events.at(-1).clientId).toBe("owner");
  ...
  expect(released.session.holder).toBeNull();
});

test("API preserves the first pending keyboard request under concurrent requests", async ({
  page,
}) => {
  const created = await sessionPost(page, {
    action: "create",
  ...
  await expectBadPost(
    page,
    { action: "request", code, clientId: "charlie", name: "Charlie" },
    "A keyboard request is already pending",
  );
  const current = await sessionGet(page, {
    code,
    clientId: "owner",
    name: "Owner",
  });
  expect(current.session.pendingRequest.name).toBe("Bob");
});

test("API parses timer flags and kick targets without truthy-string surprises", async ({
  page,
}) => {
  const created = await sessionPost(page, {
    action: "create",
  ...
  const code = created.code;
  expect(created.session.timerEnabled).toBe(false);
  ...
  expect(updated.session.timerEnabled).toBe(false);
  await expectBadPost(
    page,
    {
      action: "kick",
